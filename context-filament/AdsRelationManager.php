...
class AdsRelationManager extends RelationManager
{
    ...
    public function table(Table $table): Table
    {
        ...
        return $table
            ->recordTitleAttribute('ad_name')
            ->modifyQueryUsing(function ($query) {
                return $query->addSelect([
                    'conversion_realtime' => \App\Models\EventClick::query()
                        ->selectRaw('count(*)')
                        ->whereColumn('event_clicks.ad_id', 'ad_insights_campaigns.ad_id')
                        ->where('event_clicks.type', 'click_ad')
                        ->whereRaw('DATE(event_clicks.created_at) = ad_insights_campaigns.date_start')
                    ...
                    'rpc_est' => AllReport::query()
                        ...
                        , 0)')
                        ->whereColumn('all_reports.campaign_id', 'ad_insights_campaigns.campaign_id')
                        ->whereColumn('all_reports.date_start', 'ad_insights_campaigns.date_start')
                ...
                ToggleColumn::make('status')
                    ...
                    ->updateStateUsing(function ($record, $state) {
                        try {
                            ...
                            $success = $statusService->updateAdStatus($record->ad_id, $newStatus);

                            if ($success) {
                                $record->update(['status' => $newStatus]);
                                Notification::make()
                                    ->title('Ad status updated successfully')
                                    ->body("Ad {$record->ad_name} is now {$newStatus}")
                                    ->success()
                                    ->send();
                            } else {
                                Notification::make()
                                    ->title('Failed to update ad status')
                                    ->body("Could not update ad {$record->ad_name}")
                                    ->danger()
                                    ->send();
                            }
                    ...
                    ->tooltip(function ($record) {
                        ...
                        $isPaused = $record->status === 'PAUSED';
                        if (! $isActive && ! $isPaused) {
                            return 'Cannot toggle this ad status';
                        }

                        return $isActive ? 'Click to pause ad' : 'Click to activate ad';
                    }),
                ...
                Filter::make('ad_id')
                    ...
                    ->query(function (Builder $query, array $data): Builder {
                        return $query
                            ->when(
                                $data['ad_id'],
                                fn(Builder $query, $adId): Builder => $query->where('ad_id', 'like', "%{$adId}%"),
                            );
                    }),
                ...
                Filter::make('ad_name')
                    ...
                    ->query(function (Builder $query, array $data): Builder {
                        return $query
                            ->when(
                                $data['ad_name'],
                                fn(Builder $query, $adName): Builder => $query->where('ad_name', 'like', "%{$adName}%"),
                            );
                    }),
