...
import type { ChannelBulkCreateFormValues } from '@/features/channels/types'

type BulkCreateChannelDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  form: UseFormReturn<ChannelBulkCreateFormValues>
  submitting: boolean
  formError: string | null
  importErrors: string[]
  importSuccessCount: number
  onSubmit: (values: ChannelBulkCreateFormValues) => void | Promise<void>
}

export function BulkCreateChannelDialog({
  open,
  onOpenChange,
  form,
  submitting,
      ...
      <DialogContent className="sm:max-w-2xl max-h-[92vh] overflow-y-auto">
        ...
        {importSuccessCount > 0 ? (
          <div className="flex items-start gap-2 rounded-md border border-green-300 bg-green-50 p-3 text-sm text-green-700">
            ...
            <p>{importSuccessCount} channel(s) created successfully.</p>
          </div>
        ) : null}
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      rows={20}
                      className="min-h-50"
                      placeholder={'Channel 1|1123123123\nChannel 2|1123123124'}
                      {...field}
                    />
                  </FormControl>
                  <FormMessage />
