...
type AdsColDef = {
  key: string
  label: string
  orderBy?: AdsSelectorOrderBy
  className?: string
  render: (row: AdsSelectorRow) => React.ReactNode
}
...
function createDefaultFilters(): AdsSelectorFilterParams {
  return {
    page: 1,
    per_page: 30,
  }
...
const ADS_COLUMNS: AdsColDef[] = [
  {
    key: 'ad_id',
    label: 'Ad ID',
    orderBy: 'ad_id',
    className: 'min-w-[180px]',
    render: (row) => <span className="font-mono text-xs text-foreground">{row.ad_id ?? '—'}</span>,
  },
  {
    key: 'ad_name',
    label: 'Name',
    ...
    render: (row) => (
      <div className="flex flex-col gap-0.5">
        <span className="font-medium text-foreground">{row.ad_name ?? '—'}</span>
        <span className="text-[11px] text-muted-foreground">
          {row.account_id ?? '—'} · {row.campaign_id ?? '—'} · {row.adset_id ?? '—'}
        </span>
      </div>
    ),
  },
...
type SortHeaderProps = {
  col: AdsColDef
  orderBy: AdsSelectorOrderBy | null | undefined
  order: 'asc' | 'desc' | null | undefined
  onSort: (orderBy: AdsSelectorOrderBy | null, order: 'asc' | 'desc' | null) => void
}
...
function AdsSelectorTabInner({
  open,
  active,
  accountOptions,
  campaignOptions,
}: AdsSelectorTabProps) {
  const [filters, setFilters] = useState<AdsSelectorFilterParams>(createDefaultFilters)
  const [copyDialogOpen, setCopyDialogOpen] = useState(false)
  const [searchInput, setSearchInput] = useState('')
  const [data, setData] = useState<AdsSelectorRow[]>([])
  const [rowCount, setRowCount] = useState(0)
  const [fetching, setFetching] = useState(false)

  const loadData = useCallback(async (activeFilters: AdsSelectorFilterParams) => {
    try {
      setFetching(true)
      const { data: response } = await campaignReportApi.listAdsSelector(activeFilters)
  ...
  const onSort = useCallback((orderBy: AdsSelectorOrderBy | null, order: 'asc' | 'desc' | null) => {
    setFilters((prev) => ({
      ...prev,
  ...
  const onFilterApply = useCallback((values: Record<string, unknown>) => {
    ...
    setFilters((prev) => ({
      ...
      adset_id: parseStringOrNull(values.adset_id),
  ...
  const adIdsPreview = useMemo(
    () =>
      data
        .map((row) => row.ad_id?.trim())
    ...
    <>
        ...
        <div className="relative min-h-0 flex-1 overflow-auto">
          ...
          <Table className="text-[13px]">
            ...
            <TableBody>
                ...
                <TableRow className="h-24 border-border/70 hover:bg-transparent">
                  ...
                  >
                    <div className="flex flex-col items-center gap-2 text-muted-foreground">
                      ...
                      <p className="text-sm">No ads reports</p>
                    </div>
                  </TableCell>
                </TableRow>
              )}
              ...
              {data.map((row) => (
                <TableRow
                  key={`${row.ad_id ?? 'unknown'}-${row.date_start ?? 'unknown'}-${row.account_id ?? 'unknown'}`}
      ...
      <Dialog open={copyDialogOpen} onOpenChange={setCopyDialogOpen}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>Copy Ad IDs</DialogTitle>
          </DialogHeader>

          <div className="space-y-2">
            <p className="text-xs font-semibold text-muted-foreground">
              Ad IDs {data.length > MAX_COPY_IDS ? `(First ${MAX_COPY_IDS})` : ''}
            </p>
            <Textarea
              value={adIdsPreview}
              readOnly
...
export const AdsSelectorTab = memo(AdsSelectorTabInner)
