  ...
  const columns = useMemo<MRT_ColumnDef<RevenueReportRow>[]>(
    () => [
      ...
      { accessorKey: 'ad_id', header: 'Ad ID', size: 150 },
      {
        ...
        size: 140,
        Cell: ({ row }) => (
          <Badge variant="secondary">${Number(row.original.revenue).toFixed(4)}</Badge>
        ),
        Footer: () => <Badge>${Number(summary.revenue ?? 0).toFixed(4)}</Badge>,
      },
      {
      ...
      {
        accessorKey: 'revenue_received_at',
        header: 'Revenue Received At',
        size: 190,
      },
    ],
    [summary],
