...
1. **`fe-project-conventions`** — **`/.agents/skills/fe-project-conventions/SKILL.md`** (canonical; mirrored at **`.claude/skills/fe-project-conventions/SKILL.md`**).  
   Covers **`features/<domain>/pages`**, **`features/<domain>/components`**, routing, API client, permissions (string slugs / `PermissionSlugs`), aliases, and **Re-render optimization** (Zustand selectors, `memo`, `useCallback`, `useMemo`).
