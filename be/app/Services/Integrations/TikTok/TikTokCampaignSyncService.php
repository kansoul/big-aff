...
{
    /**
     * Sync campaign insights từ TikTok cho một account cụ thể hoặc tất cả account.
     *
    ...
    public static function sync(array $data, mixed $accountRecord = null): void
    {
        ...
        if (isset($data['account_id']) && ! empty($data['account_id'])) {
            $query = Account::whereIn('account_id', $data['account_id'])
                ->where('ads_type', AdsType::TIKTOK->value)
                ->where('status', 'ACTIVE');
        ...
        $accounts = $accountFilters
            ...
            : ($accountRecord
                ? [$accountRecord]
                : self::campaignSyncAccountsQuery(AdsType::TIKTOK->value)->get());
        ...
        foreach ($accounts as $account) {
            try {
                ...
                $insightsData = DB::transaction(function () use ($insights, $campaigns, $owner) {
                    ...
                    $insightsData = array_map(function ($insight) use ($owner) {
                        return [
                            ...
                            'reach' => $insight['reach'],
                            'ad_clicks' => $insight['ad_clicks'],
                            'cpa' => $insight['cpa'],
                            'search_clicks' => $insight['search_clicks'],
                ...
                Log::error('Error processing TikTok account '.$account->account_id.': '.$th->getMessage());
